...
\input{equations/finitevolume/cylindrical-domain}
...
\input{figures/finitevolume/mesh-types}
...
\input{figures/finitevolume/cell-anatomy-tensor}
...
\input{figures/finitevolume/cell-anatomy-curvi}
...
\input{equations/finitevolume/vectorization}
...
\input{equations/finitevolume/numbers}
...
\input{figures/finitevolume/cell-anatomy-tree}
...
\input{equations/electromagnetics/static/dcresistivity}
...
\input{figures/dc/dc-derivation}
...
\input{equations/weakform/innerproduct}
...
\input{equations/electromagnetics/static/dcresistivity-weak}

where $\vec{f}$ is the test function. We can now employ a vector identity:

\input{equations/identities/divergence}
...
\input{equations/electromagnetics/static/dcresistivity-weak-div}
...
\input{equations/electromagnetics/static/dcresistivity-weak-dirichlet}
...
\input{equations/finitevolume/divergence-1d-discrete}
...
\input{equations/finitevolume/kron3}

where $\mathbf{D}_i$ is the difference matrix in one-dimension for the $i$\textsuperscript{th} dimension. The $\mathbf{I}_i$ is the identity matrix that has the length of the cells in the $n$\textsuperscript{th} dimension. Here the full divergence operator can be formed by:

\input{equations/finitevolume/divergence-1d-discrete-together}
...
\input{figures/finitevolume/divergence}
...
\input{equations/finitevolume/curl-discrete}
...
\input{figures/finitevolume/curl}
...
\input{equations/finitevolume/grad}

In Cartesian coordinates ($d=3$), this has the much more familiar form:

\input{equations/finitevolume/grad-cart}
...
\input{equations/finitevolume/grad-discrete}
...
\input{equations/weakform/cell-innerproduct}
...
\input{equations/weakform/face-innerproduct-cartesian}
...
\input{equations/weakform/projection}
...
\input{equations/weakform/projection-expanded}
...
\input{equations/weakform/face-innerproduct}
...
\input{equations/weakform/projection-edges}
...
\input{equations/weakform/edge-innerproduct}
...
\input{equations/weakform/face-innerproduct-tensor}
...
\input{equations/weakform/face-innerproduct-tensor-av}
...
\input{equations/weakform/face-innerproduct-tensor-av-kron}
...
\input{equations/finitevolume/anisotropy-3d}

In 2D, these conventions are similarly defined as:

\input{equations/finitevolume/anisotropy-2d}
...
\input{figures/finitevolume/anisotropy-innerproduct}
...
\input{equations/weakform/face-innerproduct-Pi}

Here, we will let $\mathbf{P}_i\mathbf{w}=\mathbf{y}$ and $\mathbf{y}$ will have the form:

\input{equations/weakform/face-innerproduct-Pw}
...
\input{equations/weakform/face-innerproduct-PTsigy}
...
\input{equations/weakform/face-innerproduct-dsig1}

Meanwhile, the derivative for $\frac{\partial}{\partial\boldsymbol{\sigma}_4}$ is:

\input{equations/weakform/face-innerproduct-dsig4}
...
\input{equations/weakform/face-innerproduct-tensor-deriv}
...
\input{equations/identities/commutative-matrix}
...
\input{figures/finitevolume/classes}
...
\input{figures/finitevolume/examples/tensor-curvilinear-compare}
...
\input{figures/finitevolume/examples/tensor-curvilinear-compare-norms}
